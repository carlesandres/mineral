...
const Modal = forwardRef((props: ModalProps, ref: Ref<HTMLDivElement>) => {
  ...
  const renderedTitle = title ? (
    <Dialog.Title
      className="border-b border-[var(--border-soft-color)]
        bg-[var(--solid-bg-color)] p-4
        font-semibold "
    >
      {title}
    </Dialog.Title>
    ...
    >
      <div
        className="fixed inset-0 flex items-center justify-center
          bg-black/80 p-4 font-mono backdrop-blur-sm
          backdrop-filter"
      >
        <Dialog.Panel
          className={`
              ...
              max-w-md flex-col
              overflow-hidden rounded border border-[var(--border-color)]
              bg-[var(--editor-bg-color)]
              text-[var(--text-color)] sm:max-h-[calc(100vh-8rem)]
            ${className}`}
        >
